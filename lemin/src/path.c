...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/19 11:19:52 by yzungula          #+#    #+#             */
/*   Updated: 2019/11/15 17:18:12 by vgongora         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			add_room_to_path(t_path *tmp_room, t_path *new_p, t_link *link)
{
...
int				find_end(t_farm *farm, t_room *cur_room, t_path *path)
{
	...
	while (link)
	{
		if (not_in_path(farm->start_name, link->room->name, path))
		{
			...
			add_room_to_path(tmp_room, new_p, link);
